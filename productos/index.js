...
module.exports = {
    ...
    listar: function(productos = this.productos) {
        ...
        productos.forEach(({id,nombre,marca,precio,stock}) => {
            console.log(`${id} - ${nombre} ${marca} ${precio} | ${stock ? 'OK': 'SIN STOCK'}`)
        });
        console.log('************'.yellow)
        return null
    },
    agregar: function(nombre,marca,precio,descuento){
        ...
        return `El producto ${nuevoProducto.nombre} | ${nuevoProducto.marca} se registro `
    },
    ...
    editar: function(id){
        ...
        return `El producto ${nombre} | ${marca} se modifico sastisfactoriamente `
    }
